...
if client_selected:
    ...
    for ticker in client_selected:
        comp = yf.Ticker(f'{ticker}.NS')
        df = comp.history(interval='1d', period='5y')['Close']
        dataframe[ticker] = df
    ...
    nifty_df = nifty.history(interval='1d', period='5y')['Close']
    ...
    st.subheader("LSTM Forecast for Portfolio Cumulative Returns")
    ...
    with st.expander("Portfolio LSTM Model Summary"):
    ...
    last_date = portfolio_cum_index[-1]
    ...
    # === LSTM Forecast Nifty on cumulative returns ===
    st.subheader("LSTM Forecast for Nifty Cumulative Returns")
    ...
    with st.expander("Nifty LSTM Model Summary"):
